...
it('redirects to the post show page', function () {
    ...
    actingAs($comment->user)
        ->put(route('comment.update', $comment), ['body' => 'This is the new body'])
...
it('redirects to the correct page of the comments', function () {
    ...
    actingAs($comment->user)
        ->put(route('comment.update', ['comment' => $comment, 'page' => 2]), ['body' => 'This is the new body'])
...
it('cannot update a comment from another user', function () {
    ...
    actingAs(User::factory()->create())
        ->put(route('comment.update', ['comment' => $comment]), ['body' => 'This is the new body'])
        ->assertForbidden();
});
...
it('requires a valid body', function ($body) {
    ...
    actingAs($comment->user)
        ->put(route('comment.update', ['comment' => $comment]), ['body' => $body])
        ->assertInvalid('body');
})->with([
    null,
    true,
